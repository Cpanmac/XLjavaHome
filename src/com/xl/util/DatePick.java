...
public class DatePick {
	...
	public long countWeeksInMonth(String date) {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		if (sbegin != null && !"".equals(sbegin) && send != null
		        && !"".equals(send)) {
			try {
	...
	public String getWeekDate(String date, int weekNo, String meth) {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		Calendar cal = Calendar.getInstance();
		if (weekNo > 0 && weekNo <= weeks && sbegin != null
		        && !"".equals(sbegin)) {
			try {
	...
	public boolean weekInScopeFromNow(Date date, int week, int scope) {
		return this.weekInScopeFromNow(this.convertDateToString(date), week,
		        scope);
	}
	...
	public boolean monthInScopeFromNow(String date, int scope) {
		...
		if (scmonth > Long.MIN_VALUE) {
			if ((scope <= scmonth && scmonth <= 0)
			        || (0 <= scmonth && scmonth <= scope)) {
				res = true;
			}
		}
		return res;
	...
	public boolean quarterInScopeFromNow(Date year, int quarter, int scope) {
		return this.quarterInScopeFromNow(this.convertDateToString(year),
		        quarter, scope);
	}
